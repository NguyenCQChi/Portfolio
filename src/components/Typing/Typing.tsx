...
const Typing = ({ headings } : TypingHeadingProps) => {
  const [ typingMode, setTypingMode ] = useState<TypingMode>('typing');
  const [ headingIndex, setHeadingIndex ] = useState(0);
  const prevHeadingIndex = usePreviousState(0, headingIndex);
  ...
  const updateTypingModeTimer = useCallback(() => {
    if(typingMode == 'typing') {
      const typeDuration = (currentHeading.length * typingSpeed + delay) * 1000;
    ...
    const deleteDuration = prevHeading.length * deleteSpeed * 1000;

    return setTimeout(() => {
      setTypingMode('typing')
    }, deleteDuration)
  }, [currentHeading.length, headingIndex, headings.length, prevHeading.length, typingMode])
  ...
  return (
    <div className="mb-4 text-5xl font-bold text-red-900 md:text-5xl">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {currentHeading.split('').map((char, index) => (
            <motion.span
              key={`${char}-${index}`}
              className="initial-block whitespace-pre"
